...
def extract_bank_question_to_gen_payload(bank_question: Dict[str, Any], request_type: QuestionRequestType) -> Dict[str, Any]:
    """
...
def fetch_questions_from_bank(
    ...
    # Helper to run query
    def run_query(target_diff: Optional[str] = None):
        ...
        map_query = supabase_client.table("bank_questions_concepts_maps")\
        ...
        # 2. Fetch questions
        query = supabase_client.table("bank_questions")\
    ...
    if len(questions) < count:
        logger.info(f"Not enough {request_type.value} questions with difficulty {difficulty}. Found {len(questions)}. Relaxing difficulty.")
