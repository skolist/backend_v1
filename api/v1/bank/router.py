...
from api.v1.qgen.generate_questions.utils.fetch_questions import (
    QuestionRequestType,
    extract_bank_question_to_gen_payload,
)
...
router = APIRouter(prefix="/bank", tags=["bank_management"], dependencies=[Depends(require_admin)])
...
class BankFilter(BaseModel):
    ...
    is_solved_example: bool | None = None
    is_from_exercise: bool | None = None
    is_image_needed: bool | None = None
...
async def list_bank_questions(
    ...
    try:
        ...
        if request.filters.concept_ids and len(request.filters.concept_ids) > 0:
            map_res = (
                supabase.table("bank_questions_concepts_maps")
        ...
        for item in response.data:
            ...
            item_req_type = (
                ...
                if item.get("is_solved_example")
                else QuestionRequestType.EXERCISE_QUESTION
            )
...
async def preview_auto_correct(
    ...
    try:
        ...
        return CompareResponse(original=request.question, new=merged_new)
    except Exception as e:
